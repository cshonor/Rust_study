...
**关键**：栈从**高地址向低地址**长；堆从**低地址向高地址**长；中间留**未映射或 guard 区**，两者相向扩展，避免立刻撞车。
...
高地址 ┌─────────────────────────────┐
       ...
       │         ↓ 向下增长          │
       │                             │
       │    （空闲 / guard 页）        │  ← 栈太深或堆太多会逼近这里
       │                             │
       │         ↑ 向上增长          │
       │            Heap             │  Box / Vec 的 payload
       ├─────────────────────────────┤
       │              BSS            │  零初始化静态（加载时整段清 0）
       ├─────────────────────────────┤
       │             Data            │  已初始化静态（初值来自可执行文件）
       ├─────────────────────────────┤
       │        Code / Text          │  机器指令，只读，可共享
低地址 └─────────────────────────────┘
```

```mermaid
flowchart TB
    STK["Stack ↓ 向下<br/>局部变量 · 返回地址 · 参数"]
    GAP["guard / 未映射区<br/>栈堆相向时的缓冲"]
    HEAP["Heap ↑ 向上<br/>动态分配"]
    BSS["BSS · 零初始化静态"]
    DATA["Data · 已初始化静态"]
    TEXT["Text · 只读机器码"]
    STK --> GAP --> HEAP --> BSS --> DATA --> TEXT
```
...
| 情况 | 后果 |
|------|------|
| 递归过深、栈帧过大 | 栈一直**往下**压 → **栈溢出 (stack overflow)** |
| 堆不断分配不释放 | 堆一直**往上**顶 → 内存耗尽或与栈保护区冲突 |
| 两者逼近中间 | 分配失败 / 崩溃（具体行为依 OS 与分配器） |

Rust：**无限递归**常先栈溢出；**泄漏**让堆涨（safe Rust 少见泄漏，但 `Rc` 循环等仍可能）。
...
| **向上增长** | 虚拟地址趋势向高地址（由分配器管理，不必逐字节连续） |
