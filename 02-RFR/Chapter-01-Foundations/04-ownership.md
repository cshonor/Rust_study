...
**后声明的先 drop，先声明的后 drop。**

```rust
fn main() {
    let a = String::from("A"); // 先声明
    let b = String::from("B"); // 后声明
} // 离开作用域：先 drop b，再 drop a
```

**原因**：后声明的绑定可能依赖先声明的（`let a = ...; let r = &a;`）。若先 drop `a`，`r` 可能悬垂 → 故**先 drop 后声明的**（含引用绑定），再 drop 先声明的所有者。

```rust
let x = String::from("x");
let r = &x;   // r 后声明 → r 先「结束」→ 再 drop x
```

### 2. 复合类型内部字段：**正序（定义顺序）**

结构体、元组、数组元素等：**先定义的字段先 drop**。
...
|------|-----------|------|
| **多个局部变量** | 逆声明 | 后声明先死 |
| **同一 struct 内字段** | 正定义序 | 先定义先死 |
| **函数参数** | 逆参数列表顺序 | 与局部变量相同 |

**⚠️ 两者方向相反** — 不要混成「全是逆序」或「全是正序」。
