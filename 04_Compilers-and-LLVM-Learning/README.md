...
| 阶段 | 做什么 | 书目 / 入口 |
|------|--------|-------------|
| **立刻、零成本** | 前端直觉：词法 / 语法 / 解释器 | [**Crafting Interpreters** 中文在线](./01_Crafting-Interpreters/README.md) |
...
| **以后深入** | 优化理论、后端、读 LLVM Pass | [**鲸书**](./02_Compiler-Principles/README.md) · 配 **04** 的 ch07 / `ir_samples` |

```text
现在：01 CI 中文在线（免费）
  → 买一本：03 《自制编译器》
  → 并行：04 Learn LLVM 17 + RFR 第 2 章
以后：02 鲸书（优化地图）+ LLVM Pass / O0 vs O3 diff
```

龙书 / 虎书等**暂不列入主线**；需要补全理论地图时再开 `02` 笔记扩展。
...
| **2** | [02_Compiler-Principles](./02_Compiler-Principles/) | **鲸书**《高级编译器设计与实现》 | **以后** · 占位 |
