...
**本章定位**：**STD 库进程专题**（原书约 p.245+）— **10.1 匿名管道** · **10.2 stdio 重定向** · **10.3 双层 `process`（sys + 公开 API）**。当前笔记来自 **目录脉络 + `library/std` 公开结构**；**原书正文 unsafe 走读待刷书**。

**原书主线（脉络已写入）**：IPC 管道 → FD 重定向 → OS 适配层 / 对外 `Command`。
...
|:---:|------|------|
| **10.1** | 匿名管道 | ✅ 脉络 · 源码待刷书 |
| **10.2** | 重定向实现分析 | ✅ 脉络 · 源码待刷书 |
| **10.3** | 进程管理 | ✅ |
| **10.3.1** | OS 相关适配层 | ✅ 脉络 · 源码待刷书 |
| **10.3.2** | 对外接口层 | ✅ 脉络 · 源码待刷书 |
